...
After creating a VM2Cloud VE cluster, additional nodes can be added to expand the environment. Once a node joins the cluster, it becomes part of the same management domain and can be managed from the VM2Cloud VE interface.
...
* Expand your VM2Cloud VE infrastructure.
...
* VM2Cloud VE is installed on the new node.
* The node is not already a member of another cluster.
* All nodes are running the same VM2Cloud VE version.
...
1. Log in to the VM2Cloud VE web interface of the existing cluster.
2. Select **Datacenter**.
3. Click **Cluster**.
...
1. Open the VM2Cloud VE web interface of the new server.
...
| Version mismatch                        | Ensure all nodes are running the same VM2Cloud VE version.                    |

---

# Summary

The node has now been successfully added to the VM2Cloud VE cluster. Once the join process is complete, the new node becomes part of the shared environment and can be managed from the same VM2Cloud VE interface. You can now deploy workloads on the new node or continue expanding the cluster by adding additional nodes.
